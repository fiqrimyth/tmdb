...
class Movies with ChangeNotifier {
  ...
  Future<void> fetchInTheaters(int page) async {
    final url = '$BASE_URL/movie/now_playing?api_key=$API_KEY&page=$page';
  ...
  Future<void> fetchUpcoming(int page) async {
    final url = '$BASE_URL/movie/upcoming?api_key=$API_KEY&page=$page';
  ...
  Future<void> fetchForKids(int page) async {
    final url =
        '$BASE_URL/discover/movie?api_key=$API_KEY&language=en-US&sort_by=popularity.desc&page=$page&with_genres=16';
  ...
  Future<void> getMovieDetails(int id) async {
    final url =
        '$BASE_URL/movie/$id?api_key=$API_KEY&language=en-US&append_to_response=credits,similar,reviews,images&include_image_language=en,null';
    try {
      final response = await http.get(url);
  ...
  Future<void> getGenre(int id, int page) async {
    final url =
        '$BASE_URL/discover/movie?api_key=$API_KEY&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=$page&with_genres=$id';
    final response = await http.get(url);
    final responseData = json.decode(response.body) as Map<String, dynamic>;
    final moviesData = responseData['results'];
